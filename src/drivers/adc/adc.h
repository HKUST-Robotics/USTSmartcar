...
//ADC通道
//提醒，用的時候，可以直接用 DP0  代替 DAD0 ，其他的也類似，因為有個宏定義：#define DP0       DAD0
typedef enum ADC_Ch
{
            //SC1n[DIFF]= 0
            //  ------ADC0------野火開發板絲印層----       ------ADC1------野火開發板絲印層----
  DAD0=0,   //	    ADC0_DP0				                    ADC1_DP0
  DAD1=1,   //	    ADC0_DP1				                    ADC1_DP1
  DAD2=2,   //	    PGA0_DP					                    PGA1_DP
  DAD3=3,   //	    ADC0_DP3				                    ADC1_DP3

  //ADCx_CFG2[MUXSEL] 位決定 ADCx_SEn 通道為 a 或 b.
...
extern u16      ad_once   (ADCn,ADC_Ch,ADC_nbit);      //采集一次一路模擬量的AD值

extern u16      ad_mid    (ADCn,ADC_Ch,ADC_nbit);      //中值濾波後的結果
extern u16      ad_ave    (ADCn,ADC_Ch,ADC_nbit,u8 N); //均值濾波後的結果

extern void     adc_start (ADCn,ADC_Ch,ADC_nbit);      //開始adc轉換
extern void     adc_stop  (ADCn);                      //停止ADC轉換


//內部函數聲明
static void adc_config_alt(ADC_MemMapPtr adcmap, tADC_ConfigPtr ADC_CfgPtr);//將adc寄存器結構體配置進adc寄存器
